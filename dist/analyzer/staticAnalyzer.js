...
class StaticAnalyzer {
    ...
    calculateScore(patterns) {
        let score = 100;
        for (const pattern of patterns) {
            if (pattern.includes('setInterval'))
                score -= 10;
            if (pattern.includes('setTimeout'))
                score -= 5;
            if (pattern.includes('inefficient loop'))
                score -= 5;
            if (pattern.includes('unoptimized images'))
                score -= 10;
            if (pattern.includes('document.write'))
                score -= 10;
            if (pattern.includes('inline styles'))
                score -= 5;
            if (pattern.includes('CSS animations or transitions'))
                score -= 10;
            if (pattern.includes('large JavaScript libraries'))
                score -= 10;
        }
        if (score < 0)
            score = 0;
